...
public class CraftyCommand extends BaseCommand {
    ...
    public void clear(CommandSender sender, @Suggestion("worlds") String world) {
        ...
        if (items.isEmpty()) {
            sender.sendMessage(Messages.no_ground_items.getMessage("{world}", world).toAdvancedComponent());
            return;
        }
        ...
        sender.sendMessage(Messages.cleared_ground_items.getMessage(placeholders).toAdvancedComponent());
    }
